...
enable_show_points_cloud_map = 1; % 1:show points in 3D view

enable_save_image_data = 0; % 1: enable data saving, including RGB and depth data 


% intensity_threshold = 0.0; %0.01 , data points in intensity map below the value are deleted 
confidence_threshold = 0.1; %0.05 , data points in confidence map below the value are deleted 
...
% folder_path ='../MATLAB/Acquiring_photo';   % absolute folder path
currentfolder = pwd;
...
toc
    ...
    % show point cloud
if enable_show_points_cloud_map == 1
    disp('Showing points cloud...');
    tic
    try
    ...
    ylabel('y');
    zlabel('z');
    title(['World frame,Filtered points : ',num2str(numel(filtered_points)/(numRows*numCols)*100),'%']);
    axis equal;
